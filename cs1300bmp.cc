...
cs1300bmp_readfile(char *filename, struct cs1300bmp *image)
{
  ...
  if ( error ) {
	...
	image -> color[row][col].red = rarray[row * width + col];
	image -> color[row][col].green = garray[row * width + col];
	image -> color[row][col].blue = barray[row * width + col];
      }
    }
    //
    //  Free the memory.
...
cs1300bmp_writefile(char *filename, struct cs1300bmp *image)
{
  ...
  for (int row = 0; row < height; row ++ ) {
    for (int col = 0; col < width; col ++ ) {
      rarray[row * width + col] = image -> color[row][col].red;
      garray[row * width + col] = image -> color[row][col].green;
      barray[row * width + col] = image -> color[row][col].blue;
    }
  }
